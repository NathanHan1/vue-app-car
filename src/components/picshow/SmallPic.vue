    ...
    <div class="smallpics" ref="smallpics">
        <div class="huoche" ref="huoche" :style="{'left':nowPage * -320+'px'}">
            <ul v-for="i in allPage">
                ...
                >
                    <img :src="`http://localhost:${port}/carimages_small/${picshowState.id}/${picshowState.album}/${pic}`">
                </li>
            </ul>
        </div>
...
export default {
    computed: {
        ...
        port() {
            return this.$store.state.indexSereverState.SERVER_PORT;
        }
    },
    methods: {
        changeIdx(idx) {
